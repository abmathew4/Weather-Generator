...
public class WeatherGenerator {
    ...
    public static void populateLocationProbabilities(double[] drywetProbability, double[] wetwetProbability, double longitude, double latitude, double[][] drywet, double[][] wetwet) {
        StdRandom.setSeed(1680311575821L);
        for (int i = 0; i < 12; i++) {
            double drywetCount = 0.0;
            double wetwetCount = 0.0;
            double totalCount = 0.0;
    
            for (int j = 0; j < drywet[i].length; j++) {
                if (drywet[i][j] > 0.0) {
                    totalCount++;
                    if (j < drywet[i].length - 1 && wetwet[i][j + 1] > 0.0) {
                        drywetCount++;
                    }
                }
                if (wetwet[i][j] > 0.0) {
                    totalCount++;
                    if (j < wetwet[i].length - 1 && wetwet[i][j + 1] > 0.0) {
                        wetwetCount++;
                    }
                }
            }
    
            drywetProbability[i] = drywetCount / totalCount;
            wetwetProbability[i] = wetwetCount / totalCount;
        }
    }
    ...
    public static int[] forecastGenerator(double drywetProbability, double wetwetProbability, int numberOfDays) {
        ...
        boolean isWet = StdRandom.uniform() < 0.5; // 50% chance of being wet on first day
        forecast[0] = isWet ? 1 : 2;
    ...
    public static int[] oneMonthForecast(int numberOfLocations, int month, double longitude, double latitude) {
        StdRandom.setSeed(1680311575821L);
        double[][] drywet = new double[12][14];
        double[][] wetwet = new double[12][14];
        populateArrays(drywet, wetwet);
        double[] drywetProbability = new double[12];
        double[] wetwetProbability = new double[12];
        populateLocationProbabilities(drywetProbability, wetwetProbability, longitude, latitude, drywet, wetwet);
    
        int[] forecast = forecastGenerator(drywetProbability[month], wetwetProbability[month], 31);
    ...
    public static int bestWeekToTravel(int[] forecast){
        StdRandom.setSeed(1680311575821L);
        int n = forecast.length;
        int dryDays = 0;
        int maxDryDays = Integer.MIN_VALUE;
        int bestWeekStartIndex = 0;
        ...
        for (int i = 0; i < n - 6; i++) {
            ...
            if (dryDays > maxDryDays) {
                maxDryDays = dryDays;
                bestWeekStartIndex = i;
            }
            dryDays = 0;
        }
        
        return bestWeekStartIndex;
    }
